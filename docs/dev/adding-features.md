...
How to extend the compiler end-to-end: lexer through HIR lowering, MIR, and C emit.
Read [`FUTURE.md`](planning/FUTURE.md) for what is already shipped and known limitations.
Read [`VISION.md`](design/VISION.md) before adding kernel syntax that might belong in stdlib.
...
```
token ──┬──▶ lexer ──┐
        └──▶ syntax ──┴──▶ parser ──▶ ast ──▶ hir ──▶ mir ──▶ codegen ──▶ eye (bin)

diagnostics ──▶ shared by lexer / parser / hir   (off to the side)
...
- Record user-facing limitations in [`FUTURE.md`](planning/FUTURE.md).

## Adding the back half (MIR lowering, then C emit)
...
The invariant (REDESIGN I2): MIR lowering and the emitter are total. A well-typed
HIR lowers to valid MIR and any MIR emits valid C; neither rejects a program or
produces a diagnostic. All program rejection happens at or before HIR. If you find
yourself wanting to emit an error from `mir` or `codegen`, the check belongs in
HIR. Type-declaration order in the C output is handled by the object-topology pass
([TOPOLOGY.md](features/TOPOLOGY.md)), not by ad-hoc ordering in the emitter.

Add regression tests in `crates/codegen/src/core/tests.rs` (and `crates/mir` for
lowering shape). For externally visible behaviour, add or extend a test in
`tests/e2e.rs` and an `eyesrc/*.eye` fixture.
