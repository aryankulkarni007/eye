...
| ~~`let` type inference (T1)~~ RESOLVED 2026-06-18 | v0.5 | Was: untyped `let` emitted a placeholder. Now **let-from-init** is built: an untyped `let x = <init>` binds `x` to the initializer's bottom-up synthesized concrete type (no inference variables - the type already exists), wired across lowering / typeck / MIR (ledger "Let-from-init inference"). | Done. |
| Array literal element-type checking (`[1, true]`) | v0.7 | declared-element-type checking is now CLOSED (L4, `ArrayElementTypeMismatch`/T42, 2026-06-16) and T1/let-from-init is built. residual to verify: an *untyped* heterogeneous literal (`let xs = [1, true]`, no declared element type) - confirm it is rejected under the synthesized array type, or close the uniformity check. | verify against current behavior; close if still open (P0). |
...
| ~~`const` type/value checking (`const bool B = 5`)~~ RESOLVED 2026-06-16 | H0 C1 | Was: the declared type was recorded but the folded value not checked against it. Now `check_const_kind` (const_eval.rs, `ConstError::ConstTypeMismatch`/C14) requires the folded value's scalar kind to match the declared kind - the const analogue of the cast lattice. `int->float` widening and `ptr<-int` stay legal; integer range stays U2. | Done. |
