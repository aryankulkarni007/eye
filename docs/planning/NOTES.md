# Refactor scratch (SUPERSEDED)

> **Status: historical.** This is the raw planning scratch (and an external
> architecture dump) that preceded the pipeline refactor. Its conclusions were
> ratified and built: the typed-diagnostics partition shipped as
> [DIAGNOSTICS.md](features/DIAGNOSTICS.md) (Track 1), the HIR -> MIR -> codegen split
> shipped as [MIR.md](features/MIR.md) (Track 2), and the pure-HIR / one-true-HIR plus
> per-backend MIR idea is recorded in [REDESIGN.md](design/REDESIGN.md). Kept for
> provenance; do not treat as current design. For current state see
