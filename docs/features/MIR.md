...
the branch). `eyesrc/programs/wierd.eye` is the worked example above; it compiles and runs
...
// NOTE: SwitchArm stays a closed enum by design (I2 invariant). No trait-object
// extensibility here - the match extensibility seam lives at the AST→HIR
// lowering boundary, not in MIR or codegen. See KERNEL.md "Option A".
...
A related invariant: `SwitchArm` (the `case`-and-body pair) is a **closed enum**.
No `dyn` dispatch, no trait objects, no extensibility at the MIR level. The match
extensibility seam lives one layer up at the AST→HIR boundary; the macro engine
