...
`Unsupported::UnhoistedMatch` was transient: it codified the HIR ban
(`check_unhoisted_matches`) and was deleted at the MIR cutover (REDESIGN I3), so
nested value-position match now compiles. The arrays-as-struct-fields member was
likewise retired by the object-topology pass ([TOPOLOGY.md](TOPOLOGY.md)). The `U`
class therefore has no HIR members today; it remains in the taxonomy for future
deferrals.
