...
fn main() -> anyhow::Result<()> {
    ...
    // --- HIR ---
    let hir = database::lowered_file(&db, file);
    ...
    if !hir.diagnostics.is_empty() {
        diagnostics::render(
            &source,
            hir.diagnostics.clone().into_diags(),
            Some(&root),
            Some(input_path),
        );
    ...
    if cli.dump_mir {
        println!("--- MIR ---");
        dump::mir::dump_mir(&hir, &database::mir_map(&db, file));
    }
    if cli.dump_mir_raw {
        println!("--- MIR (raw) ---");
        dump::mir::dump_mir_raw(&hir, &database::mir_map(&db, file));
    }

    // --- C ---
    let c_source = database::c_code(&db, file);
    if cli.dump_c {
        println!("--- generated C ---");
        println!("{}", c_source.as_str());
    }
